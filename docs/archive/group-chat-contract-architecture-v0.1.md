...
`exemptList` 不单独成为主协议槽位。它是 `banSource` 的内部语义：
...
```solidity
interface IBanVoteWeightSource {
...
票权源必须由具体群类型定义。`GovVotedBanSource` 只负责投票记录、聚合和黑名单判断，不关心票权来自代币治理票、行动轮投票数，还是其他治理权重。

### 2.5 主协议聚合读接口

前端不应为了展示发言按钮分别拼 `scopeSource`、`banSource`、owner 校验和 active 状态。主协议提供聚合预检查：
...
- `banSource.isBanned(...) == true`，返回 `false`
- 否则返回 `true`
...
未知地址只按通用槽位地址展示；最多调用标准 `canPost(...)` / `isBanned(...)`，不做专用解析。

官方或前端信任的 source / banSource / plugin 建议实现：
...
banSource.stateVersion        -> banSource 内部状态变化
...
4. 若 banSource != address(0)，调用 isBanned(...)
5. 若 isBanned 返回 true，拒绝
...
- 实现 `IPostScopeSource`
- 对治理投票黑名单场景，实现 `IBanVoteWeightSource`
...
- 激活后不可关闭群聊
- 激活后不可重配 `scopeSource`、`banSource`、`beforePostPlugin`、`afterPostPlugin`
...
BanSource 是实际挂到 `GroupChat.banSource` 的合约。

职责：

- 查询或维护 `banList`
- 查询或维护 `exemptList`
- 在 `isBanned(...)` 中返回是否拒绝发言

治理投票型黑名单统一使用 `GovVotedBanSource`。这里的 `Gov` 表示“由投票治理产生黑名单”，不限定票权必须是代币治理票。不同群聊的差异只来自对应 `IBanVoteWeightSource`。

`GovVotedBanSource` 只允许配置每个 `groupId` 的投票权重源：

```text
voteWeightSourceOf(groupId) -> IBanVoteWeightSource
```
...
即由 Manager 同时提供 `voteWeightOf(...)`。阈值、投票期、是否支持反对票 / 撤票 / 复议等其他规则，全部在 `GovVotedBanSource` 构造时确定，部署后不按群重配。
...
GroupChat.banSource = GovVotedBanSource
GovVotedBanSource.voteWeightSource = TokenGroupChatManager
...
GroupChat.banSource = GovVotedBanSource
GovVotedBanSource.voteWeightSource = TokenGovGroupChatManager
GroupChat.beforePostPlugin = 可选
```
...
GroupChat.banSource = GovVotedBanSource
GovVotedBanSource.voteWeightSource = TokenActionGovGroupChatManager
GroupChat.beforePostPlugin = 可选
```
...
GroupChat.banSource = GovVotedBanSource
GovVotedBanSource.voteWeightSource = TokenActionGroupChatManager
GroupChat.beforePostPlugin = 可选
```

四类代币/行动去中心化群聊共用 `GovVotedBanSource`。差异只在：

- `scopeSource.canPost(...)`
- `IBanVoteWeightSource.voteWeightOf(...)`
...
AdminBanSource
  - banList = owner / delegate / admin 管
...
如后续需要成员制群聊，应新增独立 `scopeSource`，不要把成员资格混入 `AdminBanSource` 的黑名单语义。

## 7. AdminBanSource 权限模型
...
admin:
  banSource 内配置的 adminGroupIds 的当前 owner
```
...
```text
AdminBanSource -> GovVotedBanSource
```
