...
扩展能力通过 `meta`、`scopeSource`、`denySource` 与 `plugin` 完成，不污染协议最小模型。
...
| denySource | 黑名单判断源，回答某身份 / 地址是否被禁言；豁免名单是其内部规则 |
| beforePostPlugin | 发言前额外策略插件，用于 `mentionAll`、频率、内容格式等非标准拦截 |
| afterPostPlugin | 消息落链后的观察型插件 |
| ruleSlots | `scopeSource`、`denySource`、`beforePostPlugin`、`afterPostPlugin` 四个规则槽位的聚合读接口 |
...
- 群内私密消息
- 链下社交组件
- 主协议内置复杂投票审批；治理型黑名单由外置 `denySource` 实现
...
- `configVersion` 覆盖范围包括：`active`、`meta`、`delegateGroupId` 原始存储配置、`scopeSource`、`denySource`、`beforePostPlugin`、`afterPostPlugin`
...
- `active = false` 时，禁止 `setDenySource`
...
- `deactivateChat` 不得清空当前 `meta`、`delegateGroupId`、`scopeSource`、`denySource`、`beforePostPlugin`、`afterPostPlugin`
...
- `activateChat(groupId, metaKeys, metaValues, scopeSource, denySource, beforePostPlugin, afterPostPlugin, delegateGroupId_)`
...
- `activateChat`、`deactivateChat`、`setMeta`、`setMetaBatch`、`setDelegateGroupId`、`setScopeSource`、`setDenySource`、`setBeforePostPlugin`、`setAfterPostPlugin` 都属于 live 配置变更
- 同一笔交易里即使同时改多个配置项，`configVersion` 也只递增 1 次
- `metaValue` 与 `metaEntries` 始终只返回当前 live `meta`
- 历史 `meta` 变化仍通过事件追踪，不要求链上保留旧版本可读快照
- 重新 `activateChat` 时，`scopeSource` / `denySource` / `beforePostPlugin` / `afterPostPlugin` / `delegateGroupId_` 都按本次入参直接覆盖
- `scopeSource = address(0)` 表示默认开放发言
- `denySource = address(0)` 表示无黑名单
...
- 是否禁言，由该 chat 的 `denySource` 决定
- `denySource = address(0)` 时，视为无黑名单
...
- `quotedMessageIndex` 的合法性必须由主协议校验，并且校验发生在 `scopeSource`、`denySource` 与 `beforePostPlugin` 调用之前
- 若未配置 `scopeSource`、`denySource`、`beforePostPlugin`，则在通过 owner、`active`、内容长度等核心校验后默认允许发送
...
- `scopeSource` 与 `denySource` 应优先使用只读判断
...
- `canPost(...)` 是无内容预检查，只判断 active、发送身份 owner、`scopeSource` 与 `denySource`
...
- 若 `denySource.isDenied(...)` 调用失败，`canPostStatus(...)` 返回 `DenySourceFailed.selector`
...
- 未挂 `scopeSource`、`denySource`、`beforePostPlugin` 的 `active` chat 仍允许跨群发言
...
- `denySource` 判断是否禁言；豁免名单是 `denySource` 内部规则
- `beforePostPlugin` 只处理资格和黑名单之外的额外发言前规则
- `afterPostPlugin` 只能观察结果，不能修改已上链消息
- 调用顺序固定为：核心校验 -> `scopeSource` -> `denySource` -> `beforePostPlugin` -> 写消息 -> `afterPostPlugin`
- 若 `scopeSource.canPost(...)` 返回 `false`，该次 `post` 必须整体 `revert`
- 若 `denySource.isDenied(...)` 返回 `true`，该次 `post` 必须整体 `revert`
...
- 去中心化 Manager 激活群聊时，必须一次性写入 `scopeSource`、`denySource`、`beforePostPlugin`、`afterPostPlugin`，且 `delegateGroupId_ = 0`
- 去中心化 Manager 激活后，不得暴露 `deactivateChat` 路径
- 去中心化 Manager 激活后，不得暴露 `setScopeSource`、`setDenySource`、`setBeforePostPlugin`、`setAfterPostPlugin`、`setDelegateGroupId` 路径
- 去中心化 Manager 激活后，不得暴露修改 token / action scope、最近 `X` 轮、发言资格参数的方法
- 去中心化 Manager 不得暴露通用 `call` / `delegatecall` / `execute` 后门
- 去中心化 Manager 不可升级
- 治理投票型黑名单统一使用 `GovVotedDenySource`
- `GovVotedDenySource` 的可配置项仅为每个 `groupId` 对应的 `IDenyVoteWeightSource`
- 四种代币/行动去中心化群聊的 `IDenyVoteWeightSource` 默认就是对应 Manager
- `GovVotedDenySource` 的阈值、投票期、反对票 / 撤票 / 复议等规则必须在构造时确定，部署后不按群重配
- 链群群聊是链群服务者中心管理的群聊，黑名单使用服务者侧管理模型，不使用治理投票黑名单
- `activateChat`、`setScopeSource`、`setDenySource`、`setBeforePostPlugin`、`setAfterPostPlugin` 在写入非零地址时，必须执行相同的合约地址校验
...
- `setDenySource(groupId, sourceAddress)`
...
`stateVersion(...)` 与 `StateVersionChanged` 是可信模块推荐接口，不是所有 source / plugin 的强制接口。前端只有在可信 adapter 标记该模块支持时才调用和监听。
...
- `scopeSource(groupId)` / `denySource(groupId)` / `beforePostPlugin(groupId)` / `afterPostPlugin(groupId)` 在槽位未挂载时返回 `address(0)`
...
- `activateChat`、`setScopeSource`、`setDenySource`、`setBeforePostPlugin`、`setAfterPostPlugin` 在写入非零槽位地址时，若地址无代码，必须 `revert`
...
- `DenyRejected`：`canPostStatus(...)` 中 `denySource` 判定被拒绝
- `ScopeSourceFailed`：`canPostStatus(...)` 中 `scopeSource` 外部调用失败
- `DenySourceFailed`：`canPostStatus(...)` 中 `denySource` 外部调用失败
- `DefaultGroupIdAlreadySet` / `DefaultGroupIdNotSet`：默认身份注册表写入状态不合法

补充约束：

- 主协议不要求统一包装 `scopeSource`、`denySource`、`beforePostPlugin` 的错误；source / plugin 自定义错误应原样向上传播
...
- `activateChat` 成功时，除 `ChatActivate` 外，还必须为实际发生变化的 `meta`、`delegateGroupId`、`scopeSource`、`denySource`、`beforePostPlugin`、`afterPostPlugin` 发出对应差异事件
...
- `ScopeSourceSet.prevSourceAddress` 语义固定为“本次修改前生效中的 scope source”
- `DenySourceSet.prevSourceAddress` 语义固定为“本次修改前生效中的 deny source”
...
- 前端收到 `AfterPostPluginFailed` 后，应使用 `messages(groupId, messageIndex, 1, false)` 回查该条消息
...
- 前端配置同步要求：
- 只把 `ChatActivate`、`ChatDeactivate`、`MetaSet`、`DelegateGroupIdSet`、`ScopeSourceSet`、`DenySourceSet`、`BeforePostPluginSet`、`AfterPostPluginSet` 当作“配置已变更”的信号
...
- 可信 source / plugin 的内部状态事件应以 `StateVersionChanged(groupId, stateVersion)` 作为重拉信号
...
- 协议状态尽量少
- 新功能优先通过 `meta`、`scopeSource`、`denySource` 与 `plugin` 扩展
- 不把子频道、成员表、复杂治理塞进协议
...
- 消息全量上链且不可改写
- 未挂 `scopeSource`、`denySource`、`beforePostPlugin` 时默认允许跨群发言
...
- `T041`：`senderGroupId != groupId` 的跨群发言
  关键断言：在未挂 `scopeSource`、`denySource`、`beforePostPlugin` 时默认允许
...
- `T070B`：仅挂 `denySource` 时 `post`
  关键断言：`isDenied=true` 整笔 `revert`；`isDenied=false` 可继续
...
- `T073`：source / plugin 地址为 EOA 或无代码地址
  关键断言：`activateChat`、`setScopeSource`、`setDenySource`、`setBeforePostPlugin`、`setAfterPostPlugin` 均 `revert`
