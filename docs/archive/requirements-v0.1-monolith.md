...
- `canPost(...)` 不调用 `beforePostPlugin`，不检查 `content`、`mentions`、`mentionAll`、`quotedMessageIndex`
...
- 主协议必须把 `mentions`、`mentionAll` 与 `quotedMessageIndex` 原样传给 `beforePostPlugin` / `afterPostPlugin`
