...
扩展能力通过 `meta`、`scopeSource`、`banSource` 与 `plugin` 完成，不污染协议最小模型。
...
| groupId | 消息所属群聊标识，等于所属 chat 的 `groupId` |
| senderGroupId | 发消息时使用的身份 `GroupNFT` 编号，可与 `groupId` 相同或不同 |
...
| scopeSource | 基础发言资格源，回答某身份 / 地址本来是否有资格发言 |
| banSource | 黑名单判断源，回答某身份 / 地址是否被禁言；豁免名单是其内部规则 |
| beforePostPlugin | 发言前额外策略插件，用于 `mentionAll`、频率、内容格式等非标准拦截 |
| afterPostPlugin | 消息落链后的观察型插件 |
| ruleSlots | `scopeSource`、`banSource`、`beforePostPlugin`、`afterPostPlugin` 四个规则槽位的聚合读接口 |
...
- 群内私密消息
- 链下社交组件
- 主协议内置复杂投票审批；治理型黑名单由外置 `banSource` 实现
...
- `configVersion` 覆盖范围包括：`active`、`meta`、`delegateGroupId` 原始存储配置、`scopeSource`、`banSource`、`beforePostPlugin`、`afterPostPlugin`
...
- 每个槽位同一时间只允许挂载一个合约地址
- `address(0)` 表示该槽位未挂载
- `scopeSource = address(0)` 表示默认开放发言
- `banSource = address(0)` 表示无黑名单
- 插件 / source 自身状态由各自合约自己存储，主协议不存 `configData`
- `ruleSlots(groupId)` 必须一次性返回四个槽位地址，便于前端配置全量重拉
...
- 主协议支持关闭；去中心化群聊的 Manager 激活后不得暴露 `deactivateChat` 路径，因此去中心化群聊不可关闭
...
- `active = false` 时，禁止 `setScopeSource`
- `active = false` 时，禁止 `setBanSource`
- `active = false` 时，禁止 `setBeforePostPlugin`
- `active = false` 时，禁止 `setAfterPostPlugin`
- `active = false` 时，若某插件已挂载，允许该插件自身的内部配置写操作
- `active = false` 时，若某 source 已挂载，允许该 source 自身的内部配置写操作
- `deactivateChat` 不得清空当前 `meta`、`delegateGroupId`、`scopeSource`、`banSource`、`beforePostPlugin`、`afterPostPlugin`
...
- `activateChat(groupId, metaKeys, metaValues, scopeSource, banSource, beforePostPlugin, afterPostPlugin, delegateGroupId_)`
...
- 重新 `activateChat` 时，同名 `key` 若本次 `value` 与旧值相同，视为保留，不构成 `SetMeta` 差异事件
- 重新 `activateChat` 时，同名 `key` 若本次 `value` 与旧值不同，视为更新
- 任意成功改变 live 配置的交易，都必须让 `configVersion` 递增 1 次
- `activateChat`、`deactivateChat`、`setMeta`、`setMetaBatch`、`setDelegateGroupId`、`setScopeSource`、`setBanSource`、`setBeforePostPlugin`、`setAfterPostPlugin` 都属于 live 配置变更
- 同一笔交易里即使同时改多个配置项，`configVersion` 也只递增 1 次
- `metaValue` 与 `metaEntries` 始终只返回当前 live `meta`
- 历史 `meta` 变化仍通过事件追踪，不要求链上保留旧版本可读快照
- 重新 `activateChat` 时，`scopeSource` / `banSource` / `beforePostPlugin` / `afterPostPlugin` / `delegateGroupId_` 都按本次入参直接覆盖
- `scopeSource = address(0)` 表示默认开放发言
- `banSource = address(0)` 表示无黑名单
...
- 主协议内的 `chat`、`meta`、`delegateGroupId` 与规则槽位状态都挂在 `groupId` 下
- source / plugin 自身状态由各自合约按 `groupId` 自行隔离
- 消息与轮次分页按 `groupId` 归属
- `groupId` 恒等于所属 chat 的 `groupId`
...
- 同一笔交易里发出的多条 `SetMeta` 事件，必须携带同一个新 `configVersion`
...
- `active = false` 时禁止发消息
- `active = false` 时禁止除 `activateChat` 与已挂载 source / 插件内部配置写外的管理写操作
...
- 发消息时必须同时指定 `groupId` 与 `senderGroupId`
- `senderGroupId` 表示发言身份，可与 `groupId` 相同或不同
...
- `groupId` 对应 chat 必须处于 `active`
- `senderGroupId` 必须对应一个当前存在的 `GroupNFT`
- 不要求 `senderGroupId` 自己的 chat 已激活
- 是否允许 `senderGroupId` 向 `groupId` 发消息，先由该 chat 的 `scopeSource` 决定
- `scopeSource = address(0)` 时，基础发言资格默认通过
- 是否禁言，由该 chat 的 `banSource` 决定
- `banSource = address(0)` 时，视为无黑名单
- `mentionedSenderIds` 用于声明本条消息提及的 `GroupNFT` 身份列表
- `mentionedSenderIds` 数量上限固定为 `32`
- `mentionedSenderIds` 中每个 `mentionedGroupId` 都必须当前存在
- `mentionedSenderIds` 中不得出现重复 `mentionedGroupId`
- `mentionedSenderIds` 允许包含 `senderGroupId` 自己
- `mentionAll` 仅表示消息声明了“@all”语义
- `mentionAll=true` 时允许同时携带具体 `mentionedSenderIds`
- 主协议必须原样记录并透传 `mentionAll`
- 是否允许 `mentionAll`，由 `beforePostPlugin` 或外部规则决定；主协议不做额外判定
- 主协议必须支持消息引用字段 `quotedMessageIndex`
- `quotedMessageIndex == 0` 表示无引用
- `quotedMessageIndex > 0` 时，必须引用当前 `groupId` 内一条已存在历史消息
- 为保持语义简单，协议不支持引用 `messageIndex == 0` 的消息
- `quotedMessageIndex` 的合法性必须由主协议校验，并且校验发生在 `scopeSource`、`banSource` 与 `beforePostPlugin` 调用之前
- 若未配置 `scopeSource`、`banSource`、`beforePostPlugin`，则在通过 owner、`active`、内容长度等核心校验后默认允许发送
- 协议默认规则不得要求 `senderGroupId == groupId`
- 消息内容必须完整上链
...
- `post(groupId, senderGroupId, content, mentionedSenderIds, mentionAll, quotedMessageIndex)`
- `postByDefaultSender(groupId, content, mentionedSenderIds, mentionAll, quotedMessageIndex)`
- `canPost(groupId, senderGroupId, senderAddress)`
- `canPostStatus(groupId, senderGroupId, senderAddress)`
- `GroupDefaults.setDefaultGroupId(groupId)`
- `GroupDefaults.clearDefaultGroupId()`
- `GroupDefaults.defaultGroupIdOf(account)`
- `GroupDefaults.defaultGroupsOf(accounts)`

说明：

- `content` 类型为 `string`
- `mentionedSenderIds` 类型为 `uint256[]`
- `mentionAll` 类型为 `bool`
- `quotedMessageIndex` 类型为 `uint256`
- 以 `bytes(content).length <= 16384` 作为约束
- 以 `mentionedSenderIds.length <= 32` 作为约束
- `scopeSource` 与 `banSource` 应优先使用只读判断
- `beforePostPlugin` 应保持轻量，只处理资格和黑名单之外的额外规则
- `PostMessage` 事件不带完整 `content`
- 默认发言身份是全局地址级便捷状态，不属于任何 chat 的 `configVersion`
- `GroupDefaults.defaultGroupIdOf(account)` 返回当前有效默认身份；未设置或已失效都返回 `0`
- `postByDefaultSender(...)` 只是 `post(...)` 的语法糖，先通过默认身份注册表解析当前有效默认身份，再复用普通发消息流程
- `GroupDefaults.clearDefaultGroupId()` 基于原始存储判定是否可清理，而不是基于 `defaultGroupIdOf(account)` 的派生结果
- `canPost(...)` 是无内容预检查，只判断 active、发送身份 owner、`scopeSource` 与 `banSource`
- `canPost(...)` 等价于 `canPostStatus(...).allowed`
- `canPostStatus(...)` 返回不能发言的标准错误 selector，便于前端展示
- 若 `scopeSource.canPost(...)` 调用失败，`canPostStatus(...)` 返回 `ScopeSourceFailed.selector`
- 若 `banSource.isBanned(...)` 调用失败，`canPostStatus(...)` 返回 `BanSourceFailed.selector`
- `canPost(...)` 不调用 `beforePostPlugin`，不检查 `content`、`mentionedSenderIds`、`mentionAll`、`quotedMessageIndex`
- 某条具体消息最终能否发送，仍以 `post(...)` 实际执行结果为准
- `mentionAll` 按钮、特殊引用发送、内容格式提示等具体动作，不应只依赖 `canPost(...)`

`canPostStatus(...)` 的 `reasonCode` 固定为：
...
- 未挂 `scopeSource`、`banSource`、`beforePostPlugin` 的 `active` chat 仍允许跨群发言
- 协议不得额外拒绝 `senderGroupId != groupId` 的消息
- 每条消息必须可追溯到具体 `groupId`、`senderGroupId` 与 `senderAddress`
- 前端应可用 `canPost(...)` 判断普通发言按钮的基础可用性
- 前端应可用 `canPostStatus(...)` 展示普通发言不可用原因
...
- 协议只记录“有消息的 round”
- 协议必须支持在单个 `groupId` 内，按 `senderGroupId` 过滤该发送身份的全量消息并分页读取
- `messagesBySender(...)` 的结果范围是目标 `groupId` 下、`senderGroupId` 命中的全部消息，不额外按 `round` 再分段
- 协议应同时提供对应的轻量索引接口 `messageIndexesBySender(...)`
- `messageIndexesBySender(...)` 必须返回与 `messagesBySender(...)` 同一命中集合、同一顺序下的 `messageIndex` 数组
- 协议必须支持在单个 `groupId` 内，按被提及身份 `mentionedGroupId` 过滤命中消息并分页读取
- `messagesByMention(...)` 的结果范围是目标 `groupId` 下、`mentionedGroupId` 命中的全部消息，不额外按 `round` 再分段
- 协议应同时提供对应的轻量索引接口 `messageIndexesByMention(...)`
- `messageIndexesByMention(...)` 必须返回与 `messagesByMention(...)` 同一命中集合、同一顺序下的 `messageIndex` 数组
- 协议必须支持在单个 `groupId` 内，按 `mentionAll = true` 过滤命中消息并分页读取
...
- 某 `senderGroupId` 在该 `groupId` 内首次发言时，必须进入 `senderGroupIds(...)` 当前 live 列表
...
- 空 round 不进入 `rounds(...)`
- 某 round 无消息时，`messagesByRoundCount(groupId, round)` 返回 `0`
- 某 `senderGroupId` 在该群无消息时，`messagesBySenderCount(groupId, senderGroupId)` 返回 `0`
- `messagesBySender*` 中的 `senderGroupId` 仅作为过滤条件；即使传入的 `senderGroupId` 当前在 `LOVE20Group` 中不存在，未命中时也返回 `0` 或空数组，不额外 `revert`
- 某 round 无消息时，`roundInfo(groupId, round)` 不 `revert`，而是返回 `round = 入参`、`startIndex = 0`、`endIndex = 0`、`messageCount = 0`
...
- `message(groupId, messageIndex)`
- `messagesByRoundCount(groupId, round)`
- `messagesBySenderCount(groupId, senderGroupId)`
- `messagesByMentionCount(groupId, mentionedGroupId)`
- `messagesByMentionAllCount(groupId)`
- `senderGroupIdsCount(groupId)`
- `messages(groupId, offset, limit, reverse)`
- `messagesByRound(groupId, round, offset, limit, reverse)`
- `messagesBySender(groupId, senderGroupId, offset, limit, reverse)`
- `messageIndexesBySender(groupId, senderGroupId, offset, limit, reverse)`
- `messagesByMention(groupId, mentionedGroupId, offset, limit, reverse)`
- `messageIndexesByMention(groupId, mentionedGroupId, offset, limit, reverse)`
- `messagesByMentionAll(groupId, offset, limit, reverse)`
- `messageIndexesByMentionAll(groupId, offset, limit, reverse)`
- `senderGroupIds(groupId, offset, limit, reverse)`
- `roundsCount(groupId)`
- `rounds(groupId, offset, limit, reverse)`
- `roundInfo(groupId, round)`

分页语义：

- `messages(groupId, offset, limit, reverse)` 中 `offset` 为所选返回方向下的全局消息 offset
- `messagesByRound(groupId, round, offset, limit, reverse)` 中 `offset` 为所选返回方向下的该 round 内 offset
- `messagesBySender(groupId, senderGroupId, offset, limit, reverse)` 中 `offset` 为所选返回方向下的该发送者命中消息列表内 offset
- `messageIndexesBySender(groupId, senderGroupId, offset, limit, reverse)` 中 `offset` 为所选返回方向下的该发送者命中消息索引列表内 offset
- `messagesByMention(groupId, mentionedGroupId, offset, limit, reverse)` 中 `offset` 为所选返回方向下的该提及命中消息列表内 offset
- `messageIndexesByMention(groupId, mentionedGroupId, offset, limit, reverse)` 中 `offset` 为所选返回方向下的该提及命中消息索引列表内 offset
- `messagesByMentionAll(groupId, offset, limit, reverse)` 中 `offset` 为所选返回方向下的 `mentionAll=true` 命中消息列表内 offset
- `messageIndexesByMentionAll(groupId, offset, limit, reverse)` 中 `offset` 为所选返回方向下的 `mentionAll=true` 命中消息索引列表内 offset
- `senderGroupIds(groupId, offset, limit, reverse)` 中 `offset` 为所选返回方向下的该群已发言 `senderGroupId` 列表内 offset
- `rounds(groupId, offset, limit, reverse)` 中 `offset` 为所选返回方向下的“有消息轮次列表”分页位置，不是真实 `round` 编号
...
- `beforePostPlugin`
- `afterPostPlugin`
- 每个槽位同一时间只允许挂载一个合约地址，不支持链式执行
- `scopeSource` 判断基础发言资格
- `banSource` 判断是否禁言；豁免名单是 `banSource` 内部规则
- `beforePostPlugin` 只处理资格和黑名单之外的额外发言前规则
- `afterPostPlugin` 只能观察结果，不能修改已上链消息
- 调用顺序固定为：核心校验 -> `scopeSource` -> `banSource` -> `beforePostPlugin` -> 写消息 -> `afterPostPlugin`
- 若 `scopeSource.canPost(...)` 返回 `false`，该次 `post` 必须整体 `revert`
- 若 `banSource.isBanned(...)` 返回 `true`，该次 `post` 必须整体 `revert`
- 若 `beforePostPlugin.beforePost(...)` `revert`，该次 `post` 必须整体 `revert`
- 以上任一拒绝都不得留下消息、事件或被占用的 `messageIndex`
- 主协议必须把 `mentionedSenderIds`、`mentionAll` 与 `quotedMessageIndex` 原样传给 `beforePostPlugin` / `afterPostPlugin`
- `mentionAll` 的允许性由 `beforePostPlugin` 或外部规则自行判断；主协议不做额外许可判定
- 不使用 `delegatecall`
- 主协议不传 `ctx`
- source / 插件自己存状态
- 主协议不存 `configData`
- 单个 source / 插件合约可服务多个 `groupId`
- source / 插件内部状态必须按 `groupId` 自行隔离
- source / 插件配置权限默认锚定当前 chat 的 `owner` / 有效 `delegateGroupId` 当前 owner
- source / 插件内部配置写接口必须显式接收 `groupId`
- source / 插件内部配置写权限校验必须实时基于当前 chat 的 `owner` 与 `delegateGroupIdOf(groupId)`
- `active = false` 不阻止已挂载 source / 插件的内部配置写；是否最终允许，由 source / 插件自身规则决定
- 四个槽位允许指向同一个合约地址，只要该合约同时实现所需接口
- 四个槽位若非 `address(0)`，必须是已部署合约地址
- 对存在的 `GroupNFT`，若对应槽位当前未挂载，则对应读接口必须返回 `address(0)`，不 `revert`
- 去中心化群聊的规则不可变性由 Manager 约束保证，不由主协议状态位保证
- 四种代币/行动去中心化群聊必须分别对应四个 Manager
- Manager 激活函数统一命名为 `activate(...)`，但每个 Manager 的入参按群聊类型定义，不使用通用群类型参数模板
- 去中心化 Manager 激活群聊时，必须一次性写入 `scopeSource`、`banSource`、`beforePostPlugin`、`afterPostPlugin`，且 `delegateGroupId_ = 0`
- 去中心化 Manager 激活后，不得暴露 `deactivateChat` 路径
- 去中心化 Manager 激活后，不得暴露 `setScopeSource`、`setBanSource`、`setBeforePostPlugin`、`setAfterPostPlugin`、`setDelegateGroupId` 路径
- 去中心化 Manager 激活后，不得暴露修改 token / action scope、最近 `X` 轮、发言资格参数的方法
- 去中心化 Manager 不得暴露通用 `call` / `delegatecall` / `execute` 后门
- 去中心化 Manager 不可升级
- 治理投票型黑名单统一使用 `GovVotedBanSource`
- `GovVotedBanSource` 的可配置项仅为每个 `groupId` 对应的 `IBanVoteWeightSource`
- 四种代币/行动去中心化群聊的 `IBanVoteWeightSource` 默认就是对应 Manager
- `GovVotedBanSource` 的阈值、投票期、反对票 / 撤票 / 复议等规则必须在构造时确定，部署后不按群重配
- 链群群聊是链群服务者中心管理的群聊，黑名单使用服务者侧管理模型，不使用治理投票黑名单
- `activateChat`、`setScopeSource`、`setBanSource`、`setBeforePostPlugin`、`setAfterPostPlugin` 在写入非零地址时，必须执行相同的合约地址校验
- 传入 EOA 或无代码地址作为非零槽位地址时，必须 `revert`
- 将任一规则槽位重复设置为当前存储中的相同配置值时，必须 `revert`
- 任意成功改变 live 规则槽位配置的交易，都必须让 `configVersion` 递增 1 次
- 若同一笔交易同时改动多个规则槽位，`configVersion` 也只递增 1 次
- 主协议 `post` 必须 `nonReentrant`
- `afterPostPlugin` 失败不得回滚主消息，只能记录失败事件
- 主协议不强制 `afterPostPlugin` gas cap
- 原因：去中心化群聊激活前应自行验证；中心化群聊可自行替换插件
- 主协议不要求 source / plugin 实现 `moduleKind()` 或 `moduleVersion()`
- 主协议不依赖 source / plugin 自报类型做任何逻辑判断
- source / plugin 的专用展示解析由前端自己决定
- 前端可信地址表必须按 `chainId + 合约地址` 匹配，不得只按地址匹配
...
- `scopeSource(groupId)`
- `banSource(groupId)`
- `beforePostPlugin(groupId)`
- `afterPostPlugin(groupId)`
- `ruleSlots(groupId)`
- `canPost(groupId, senderGroupId, senderAddress)`
- `canPostStatus(groupId, senderGroupId, senderAddress)`
...
`stateVersion(...)` 与 `ChangeStateVersion` 是可信模块推荐接口，不是所有 source / plugin 的强制接口。前端只有在可信 adapter 标记该模块支持时才调用和监听。
...
- `scopeSource` 应能限制哪些人本来有资格发消息
- `banSource` 应能限制哪些人被禁言
- 插件应能支持审核、同步、镜像等扩展
- 不同实现不得各自定义不兼容的 source / hook 核心上下文字段
- 前端不需要相信任意第三方合约的自报类型
...
- `scopeSource(groupId)` / `banSource(groupId)` / `beforePostPlugin(groupId)` / `afterPostPlugin(groupId)` 在槽位未挂载时返回 `address(0)`
- `GroupDefaults.defaultGroupIdOf(account)` 在未设置或已失效时返回 `0`
- `message(groupId, messageIndex)` 在 `messageIndex` 越界时必须 `revert`
- `messagesByRoundCount(groupId, round)` 对无消息 round 返回 `0`
- `messagesBySenderCount(groupId, senderGroupId)` 对该发送身份无消息时返回 `0`
- `senderGroupIdsCount(groupId)` 对尚无任何发言身份时返回 `0`
- `roundInfo(groupId, round)` 对无消息 round 不 `revert`，而是返回 `round = 入参`、`startIndex = 0`、`endIndex = 0`、`messageCount = 0`
...
- `post` 中 `mentionedSenderIds.length > 32` 时，必须 `revert`
...
- `activateChat`、`setScopeSource`、`setBanSource`、`setBeforePostPlugin`、`setAfterPostPlugin` 在写入非零槽位地址时，若地址无代码，必须 `revert`
...
error DelegateGroupIdCannotBeGroupId();
...
- `DelegateGroupIdCannotBeGroupId` / `DelegateGroupIdUnchanged`：代理身份写入不合法或不会改变当前存储态
- `DuplicateMentionedSenderId`：`mentionedSenderIds` 中出现重复身份
- `TooManyMentionedSenderIds`：`mentionedSenderIds` 超过固定上限 `32`
- `PluginAddressHasNoCode` / `PluginAddressUnchanged`：source / plugin 地址不合法或不会改变当前 live 配置
- `ContentEmpty` / `ContentTooLong`：消息内容不合法
- `InvalidQuotedMessageIndex`：引用目标不存在或越界
- `InvalidMessageIndex`：单条消息读取越界
- `DefaultGroupIdNotSet`：`postByDefaultSender(...)` 时默认身份未设置或已失效
- `GroupDefaultsHasNoCode`：构造主协议时传入的默认身份注册表地址无代码；`LOVE20_GROUP` 由 `GroupDefaults.GROUP_ADDRESS()` 派生
- `ScopeRejected`：`canPostStatus(...)` 中 `scopeSource` 判定无资格
- `BanRejected`：`canPostStatus(...)` 中 `banSource` 判定被拒绝
- `ScopeSourceFailed`：`canPostStatus(...)` 中 `scopeSource` 外部调用失败
- `BanSourceFailed`：`canPostStatus(...)` 中 `banSource` 外部调用失败
- `DefaultGroupIdAlreadySet` / `DefaultGroupIdNotSet`：默认身份注册表写入状态不合法

补充约束：

- 主协议不要求统一包装 `scopeSource`、`banSource`、`beforePostPlugin` 的错误；source / plugin 自定义错误应原样向上传播
- `afterPost` 失败只记录 `FailAfterPostPlugin`，不额外要求统一错误类型
...
- `PostMessage`
- `FailAfterPostPlugin`
- 默认身份注册表：`SetDefaultGroupId`
- 默认身份注册表：`ClearDefaultGroupId`
...
```solidity
event SetDefaultGroupId(
    address indexed account,
    uint256 indexed groupId
);

event ClearDefaultGroupId(
    address indexed account,
...
- `activateChat` 成功时，除 `ChatActivate` 外，还必须为实际发生变化的 `meta`、`delegateGroupId`、`scopeSource`、`banSource`、`beforePostPlugin`、`afterPostPlugin` 发出对应差异事件
- `deactivateChat` 成功时，只发出 `ChatDeactivate`
- `activateChat` 导致某个旧 `meta` key 被移除时，该移除必须视为一次删除，并发出 `SetMeta(..., value = bytes(\"\"), prevValue = oldValue)`
- `setMetaBatch` 发出的多条 `SetMeta` 事件顺序，必须与输入数组顺序一致
- `activateChat` 中，对旧 live `meta` 被移除的 `key` 发出的删除型 `SetMeta` 事件，必须按旧 live key 顺序发出
- `activateChat` 中，对显式传入 `metaKeys/metaValues` 发出的新增 / 更新型 `SetMeta` 事件，必须与输入数组顺序一致
- `activateChat` 中，对值未变化的保留 `key` 不发 `SetMeta`
...
- `SetMeta.prevValue` 语义固定为“本次修改前该 key 的有效值”
- 若本次修改前该 key 不存在，则 `SetMeta.prevValue = bytes(\"\")`
- `SetMeta.key` 不设为 `indexed`
- 原因：当前值查询应走 `metaValue` / `metaEntries`，不依赖日志按 key 过滤
- `SetScopeSource.prevSourceAddress` 语义固定为“本次修改前生效中的 scope source”
- `SetBanSource.prevSourceAddress` 语义固定为“本次修改前生效中的 ban source”
- `SetBeforePostPlugin.prevPluginAddress` 语义固定为“本次修改前生效中的 beforePost plugin”
- `SetAfterPostPlugin.prevPluginAddress` 语义固定为“本次修改前生效中的 afterPost plugin”
- `PostMessage` 不带完整 `content`
- `PostMessage` 不带 `contentHash`
- `post` 成功时，必须先完成消息状态写入并发出 `PostMessage`，再执行 `afterPost`
- 若 `afterPost` 失败，`FailAfterPostPlugin` 必须在同一交易中、且位于 `PostMessage` 之后发出
- `FailAfterPostPlugin` 用于定位失败插件对应的已落链消息
- 前端收到 `FailAfterPostPlugin` 后，应使用 `messages(groupId, messageIndex, 1, false)` 回查该条消息
...
- 必须支持按 `groupId + senderGroupId` 查询该发送身份在该群的消息分页
- 必须支持按 `groupId + senderGroupId` 查询该发送身份在该群的消息索引分页
- 必须支持按 `groupId` 查询该群已发言 `senderGroupId` 列表分页
- 必须支持不传 `round` 查询 `groupId` 下全量消息分页
- 前端消息同步要求：
- 只把 `PostMessage` 当作“发现新消息”的信号
- 正文内容始终以 `messages(...)` / `messagesByRound(...)` 的 `view` 结果为准
- 本地应维护每个 `groupId` 的最新 `messageIndex` 游标
- 收到 `PostMessage` 后：
- 若 `messageIndex == latestMessageIndex + 1`，应使用 `messages(groupId, messageIndex, 1, false)` 拉取该条消息
- 若 `messageIndex > latestMessageIndex + 1`，说明存在缺口，应按缺口区间补拉
- 前端配置同步要求：
- 只把 `ChatActivate`、`ChatDeactivate`、`SetMeta`、`DelegateGroupIdSet`、`SetScopeSource`、`SetBanSource`、`SetBeforePostPlugin`、`SetAfterPostPlugin` 当作“配置已变更”的信号
- 本地应维护每个 `groupId` 的 `configVersion` 缓存
- 收到任一配置事件后：
- 若事件里的 `configVersion` 大于本地缓存，应直接全量重拉当前配置
- 配置全量重拉应至少覆盖：`chatInfo`、`metaEntries`、`delegateGroupIdOf`、`ruleSlots`
- 若前端已适配可信 source / plugin，可按可信 adapter 读取对应 `stateVersion(groupId)` 与专用展示数据
- 未在可信地址表中的 source / plugin，不应调用其专用展示接口，也不应相信其自报类型
- 可信 source / plugin 的内部状态事件应以 `ChangeStateVersion(groupId, stateVersion)` 作为重拉信号
- 第三方模块即使发出同名事件，前端也只有在 `chainId + 合约地址` 命中可信地址表时才处理
- 发言按钮基础可用性应优先读取 `canPost(groupId, senderGroupId, senderAddress)`
- owner 同步要求：
...
- 再读取 `messagesCount(groupId)` 并对比本地 `latestMessageIndex`
...
- 无特权恢复口
- 去中心化 Manager 不可升级
- 去中心化 Manager 不得有通用外部调用后门
- 去中心化 Manager 激活群聊后，不得关闭群聊、重配槽位或修改发言资格参数
- 去中心化 Manager 的群聊激活入口必须按群聊类型专门定义

### 10.2 安全

- 管理类写接口必须做 `owner` / 有效 `delegateGroupId` 当前 owner / `active` 校验
- `activateChat`、`deactivateChat` 必须只允许 `owner`
- 发消息接口必须做 `owner` / source / plugin 校验，不得接受 `delegateGroupId` 当前 owner 冒充发言身份
- source / plugin 不能越权修改核心状态
- 代理配置要防止对错误 owner 生效
- 消息内容要有明确长度上限
- `post` 与 source / plugin 交互过程要防重入

### 10.3 可扩展

- 协议状态尽量少
- 新功能优先通过 `meta`、`scopeSource`、`banSource` 与 `plugin` 扩展
- 不把子频道、成员表、复杂治理塞进协议
...
- 消息全量上链且不可改写
- 未挂 `scopeSource`、`banSource`、`beforePostPlugin` 时默认允许跨群发言
- `meta` 能被设置、批量设置、删除、读取、分页枚举
- source / plugin 能限制发言但不能破坏核心不可变性
...
- `T020`：`setMeta` 新增 key
  关键断言：当前值更新；`configVersion` 递增；`SetMeta.prevValue=bytes("")`
- `T021`：`setMeta` 更新已有 key
  关键断言：位置不变；`SetMeta.prevValue` 为旧值
...
- `T027`：`setMetaBatch` 同时改多个 key
  关键断言：`configVersion` 仅递增 1；多条 `SetMeta` 带同一新版本号；事件顺序与输入一致
- `T028`：重新 `activateChat` 全量覆盖 `meta`
  关键断言：未传入 key 视为删除；保留 key 的相对顺序不变；新 key 追加到尾部；删除型 `SetMeta` 先于新增/更新型 `SetMeta`
...
- `T041`：`senderGroupId != groupId` 的跨群发言
  关键断言：在未挂 `scopeSource`、`banSource`、`beforePostPlugin` 时默认允许
...
- `T048`：消息携带 `mentionedSenderIds`
  关键断言：消息正文、`mentionedSenderIds`、按提及身份的消息分页与索引分页一致
- `T049`：重复 `mentionedSenderIds` 与 `mentionAll` 索引
  关键断言：重复 `mentionedGroupId` 必须 `revert`；`mentionAll=true` 的消息可被独立分页查询
- `T084`：单条消息读取
  关键断言：`message(groupId, messageIndex)` 直接返回完整消息；越界时 `revert`
- `T085`：消息引用
  关键断言：`quotedMessageIndex` 正确存储；不存在或越界引用必须 `revert`
- `T086`：`mentionedSenderIds` 超上限
  关键断言：`mentionedSenderIds.length > 32` 必须 `revert`
- `T087`：默认发言身份发消息
  关键断言：通过默认身份注册表设置默认身份后可通过 `postByDefaultSender(...)` 成功发消息
- `T088`：默认发言身份失效与恢复
  关键断言：NFT 转出后 `GroupDefaults.defaultGroupIdOf=0`；转回后自动恢复
- `T089`：默认发言身份清理
  关键断言：即使默认身份已失效，只要原始存储存在，`GroupDefaults.clearDefaultGroupId()` 仍可清理
- `T090`：默认发言身份 no-op
...
- `T070`：未挂插件时 `post`
  关键断言：默认放行
- `T070A`：仅挂 `scopeSource` 时 `post`
  关键断言：`canPost=false` 整笔 `revert`；`canPost=true` 可继续
- `T070B`：仅挂 `banSource` 时 `post`
  关键断言：`isBanned=true` 整笔 `revert`；`isBanned=false` 可继续
- `T071`：`beforePost` 明确拒绝
  关键断言：整笔 `revert`；不落消息；不发 `PostMessage`；不占用 `messageIndex`
- `T072`：`afterPost` 失败
  关键断言：主消息已落链；`PostMessage` 先发；随后发 `FailAfterPostPlugin`
- `T073`：source / plugin 地址为 EOA 或无代码地址
  关键断言：`activateChat`、`setScopeSource`、`setBanSource`、`setBeforePostPlugin`、`setAfterPostPlugin` 均 `revert`
- `T074`：关闭态下 source / 插件内部配置写
  关键断言：主协议配置写仍禁止；source / 插件内部配置写可按各自规则成功
- `T075`：source / 插件尝试重入 `post`
  关键断言：主协议 `nonReentrant` 生效
- `T076`：`afterPost` 尝试修改主协议状态
  关键断言：主消息已落链；主协议状态不被插件篡改
- `T077`：`beforePost` 接收 `mentionedSenderIds` / `mentionAll`
