...
- source / plugin 自身状态由各自合约按 `chatGroupId` 自行隔离
...
- 是否允许 `senderGroupId` 向 `chatGroupId` 发消息，先由该 chat 的 `scopeSource` 决定
...
- `post(chatGroupId, senderGroupId, content, mentionedSenderIds, mentionAll, quotedMessageIndex)`
- `postByDefaultSender(chatGroupId, content, mentionedSenderIds, mentionAll, quotedMessageIndex)`
- `canPost(chatGroupId, senderGroupId, senderAddress)`
- `canPostStatus(chatGroupId, senderGroupId, senderAddress)`
...
- source / 插件自己存状态
- 主协议不存 `configData`
- 单个 source / 插件合约可服务多个 `chatGroupId`
- source / 插件内部状态必须按 `chatGroupId` 自行隔离
- source / 插件配置权限默认锚定当前 chat 的 `owner` / 有效 `delegateGroupId` 当前 owner
- source / 插件内部配置写接口必须显式接收 `chatGroupId`
- source / 插件内部配置写权限校验必须实时基于当前 chat 的 `owner` 与 `delegateGroupIdOf(chatGroupId)`
...
- `GovVotedDenySource` 的可配置项仅为每个 `chatGroupId` 对应的 `IDenyVoteWeightSource`
...
- `setScopeSource(chatGroupId, sourceAddress)`
- `setDenySource(chatGroupId, sourceAddress)`
- `setBeforePostPlugin(chatGroupId, pluginAddress)`
- `setAfterPostPlugin(chatGroupId, pluginAddress)`
- `scopeSource(chatGroupId)`
- `denySource(chatGroupId)`
- `beforePostPlugin(chatGroupId)`
- `afterPostPlugin(chatGroupId)`
- `ruleSlots(chatGroupId)`
- `canPost(chatGroupId, senderGroupId, senderAddress)`
- `canPostStatus(chatGroupId, senderGroupId, senderAddress)`
...
- `scopeSource(chatGroupId)` / `denySource(chatGroupId)` / `beforePostPlugin(chatGroupId)` / `afterPostPlugin(chatGroupId)` 在槽位未挂载时返回 `address(0)`
...
- 若前端已适配可信 source / plugin，可按可信 adapter 读取对应 `stateVersion(chatGroupId)` 与专用展示数据
- 未在可信地址表中的 source / plugin，不应调用其专用展示接口，也不应相信其自报类型
- 可信 source / plugin 的内部状态事件应以 `StateVersionChanged(chatGroupId, stateVersion)` 作为重拉信号
- 第三方模块即使发出同名事件，前端也只有在 `chainId + 合约地址` 命中可信地址表时才处理
- 发言按钮基础可用性应优先读取 `canPost(chatGroupId, senderGroupId, senderAddress)`
- owner 同步要求：
