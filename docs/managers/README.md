...
`activate(...)` 返回新铸造并激活的 `chatGroupId`。
...
- `tokenOfChatGroup(chatGroupId)`
- `chatGroupIdOfToken(token)`
- `activatedTokensCount()`
- `activatedTokens(offset, limit, reverse)` 返回 `tokens`、`chatGroupIds`

action 类：

- `actionOfChatGroup(chatGroupId)` 返回 `token`、`actionId`
- `chatGroupIdOfAction(token, actionId)`
- `actionsCountOf(token)`
- `actionsOf(token, offset, limit, reverse)` 返回 `actionIds`、`chatGroupIds`
- `chatGroupIdsOfActions(token, actionIds)`：等长返回；未激活项为 `0`
- `actionsOfChatGroups(chatGroupIds)`：等长返回 `tokens`、`actionIds`
- `RECENT_ROUNDS()`
