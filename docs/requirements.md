...
| senderAddress | 实际发起交易的地址，必须是 `senderGroupId` 当前 owner |
...
- `chatGroupId`，消息发往的群聊标识，等于目标 chat 的 `groupId`
- `senderGroupId`，发消息时使用的身份 `GroupNFT` 编号
- `senderAddress`，实际发起交易的地址
- `round`
- `messageIndex`，目标群内全局消息序号
- `content`
- `timestamp`
- `blockNumber`
...
- `messageIndex` 以整个目标群的消息列表为准，不按 `round` 重置
...
- `chat`、`meta`、插件状态都挂在 `groupId` 下。
- 消息与轮次分页按 `chatGroupId` 归属；`chatGroupId` 恒等于目标 chat 的 `groupId`。
...
- 每个群聊必须支持 KV 元信息。
- `meta` 的 key 类型固定为 `string`，value 类型固定为 `bytes`。
- `meta` 必须能单键读取。
- `meta` 必须能分页枚举。
- `meta` 更新必须保留历史可追踪性。
- `meta` 的内容应允许承载字符串、地址、JSON 编码文本、其他协议引用。
- 插件正式配置不得编码进 `meta`，应作为独立的插件状态存储与查询。
...
- 代理不能绕过 `active` 状态。
- 代理不能代替 owner 作为 `senderGroupId` 发消息。

### 7.7 发消息

- 发消息时必须同时指定 `chatGroupId` 与 `senderGroupId`。
- `senderGroupId` 表示发言时使用的身份，可与 `chatGroupId` 相同或不同。
...
- 是否允许 `senderGroupId` 向 `chatGroupId` 发消息，由该 chat 的插件或规则决定。
- 消息内容必须完整上链。
- 单条消息内容长度上限固定为 `8KB`。
- 消息只能新增，不能编辑或删除。
- 消息必须包含 `chatGroupId`、`senderGroupId`、`senderAddress`、时间、区块号、轮次、序号等上下文。
...
- `currentRound()`
- `messageCount(chatGroupId)`
- `messageCount(chatGroupId, round)`
- `getMessages(chatGroupId, offset, limit, reverse)`
- `getMessages(chatGroupId, round, offset, limit, reverse)`
- `getRounds(chatGroupId, offset, limit, reverse)`
...
- 至少保留 `beforePost` 与 `afterPost` 两个独立 hook 槽位。
- 每个 hook 只允许挂载一个 plugin，不支持多插件链式执行。
- `beforePost` 可拒绝消息发送。
...
每个关键事件至少应携带与自身场景对应的主键和操作者字段。

- `ChatActivate`、`ChatUpdate`、`MetaSet`、`DelegateSet`、`PluginSet`、`ChatActiveChange` 至少携带 `groupId`
- 涉及 owner 或代理操作的事件，应同时携带 `owner` 或 `operator`
- `MessagePost` 至少携带 `chatGroupId`
- `MessagePost` 至少携带 `senderGroupId`
- `MessagePost` 至少携带 `senderAddress`
- `MessagePost` 至少携带 `round`
- `MessagePost` 至少携带 `index`，目标群内全局消息序号
...
- 管理类写接口必须做 owner / delegate / plugin 校验
