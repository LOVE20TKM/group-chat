...
| 术语 | 含义 |
| --- | --- |
| GroupNFT | 已部署且不可 burn 的 `LOVE20Group` 群 NFT，作为链上身份凭证；本文不抽象支持其他 ERC721 身份源 |
...
- `configVersion` 覆盖范围包括：`active`、`meta`、`delegate` 原始存储配置、`beforePostPlugin`、`afterPostPlugin`
- `configVersion` 不覆盖：实时 `owner`、消息、`firstActivated*`
- `configVersion` 不覆盖：随 `owner` 变化派生出的当前有效 `delegate`
...
- `delegateOf(groupId)` 属于运行时派生读值，不属于 `configVersion` 覆盖范围
...
- 仅当本次 `setDelegate` 不会改变当前存储态时，才允许视为重复设置并 `revert`
- 代理状态必须可查询
- 代理配置必须可清空
- `setDelegate(groupId, address(0))` 表示清空当前存储的 `delegate` 配置
- `setDelegate(groupId, address(0))` 时，必须同时清空 `delegateOwnerSnapshot`
...
- `setDelegate(groupId, delegate)` 的 no-op 判定必须基于当前存储态，而不是仅基于 `delegateOf(groupId)` 的派生结果
...
- 插件内部配置写权限校验必须实时基于当前 chat 的 `owner` 与 `delegateOf(chatGroupId)`
...
- `setDelegate` 中，若当前存储的 `delegate` 与 `delegateOwnerSnapshot` 已与本次目标存储态完全相同，必须 `revert`
...
- `NotChatOwnerOrDelegate`：要求当前 owner 或有效 delegate 的接口被未授权地址调用
...
- `DelegateCannotBeOwner` / `DelegateUnchanged`：代理写入不合法或不会改变当前存储态
...
- delegate 同步要求：
- `delegateOf(groupId)` 不属于 `configVersion` 覆盖范围
- 前端在收到 `GroupNFT` 的 `Transfer` 事件后、进入管理页前、以及发起管理交易前，必须重拉 `delegateOf(groupId)`
- 首次进入 / 重连 / 刷新后：
...
- `T002`：至少准备 4 个 `GroupNFT`
  关键断言：覆盖 chat owner、sender owner、第三方、delegate、跨群发言场景
...
- `T013`：owner `deactivateChat`
  关键断言：`active=false`；`configVersion` 递增 1；`meta`、`delegate`、插件槽位不被清空
...
### 13.4 Delegate 与 NFT 转让

- `T030`：owner 设置 delegate
  关键断言：`delegateOf(groupId)` 正确返回；`configVersion` 递增
- `T031`：delegate 设为 owner 自己
  关键断言：`revert`
- `T032`：`setDelegate` 设为当前存储态相同值
  关键断言：`revert`
- `T033`：`setDelegate(groupId, address(0))`
  关键断言：delegate 与 snapshot 同时清空
- `T034`：NFT 转给新 owner
  关键断言：旧 delegate 立即失效；`delegateOf(groupId)=address(0)`；`configVersion` 不变
- `T035`：NFT 转回原 owner
  关键断言：旧 delegate 自动恢复；主协议与插件权限同步恢复
- `T036`：前 owner 或失效 delegate 继续管理
...
- `T083`：`MessagePost` 与正文读取
  关键断言：事件仅作发现信号；正文需通过 `messages(...)` 读取并与落链状态一致

### 13.10 建议测试组织

- 单元测试优先覆盖：权限、零值规则、分页、`configVersion`、事件顺序
- 集成测试优先覆盖：NFT 转让、delegate 恢复、跨 round 发消息、插件 hook、关闭态插件配置写
- 所有新增行为都应先写失败测试，再写实现
