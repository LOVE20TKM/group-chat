...
- 手工黑名单状态读取 `GroupBanList.stateVersion(groupId)` 与 `GroupBanList.StateVersionChanged`。
...
- 展示结果可以按 `groupId + banSource + stateVersion + senderAddress/senderId` 分开缓存；监听黑名单相关事件或 `StateVersionChanged` 后清理对应缓存。
