...
`GovVotedBanSource` 负责通过投票产生黑名单。这里的 `Gov` 表示“由投票治理产生”，不限定票权必须来自 token 治理票。

不同群聊的差异来自对应 `IBanVoteWeightSource`：
...
去中心化群聊的 `groupId` owner 是对应 Manager。该 Manager 必须实现 `src/interfaces/sources/ban/IBanVoteWeightSource.sol`，为本合约提供 `voteWeightOf(...)`。
...
如果未来需要“不支持反对票”“不可撤票”“不可刷新票权”等不同治理语义，应新增独立 BanSource 合约类型，不在 `GovVotedBanSource` 内增加模式开关。
...
- `voteBy*` 会用当前票权覆盖旧立场，`supportBan=true` 表示赞成拉黑，`supportBan=false` 表示反对拉黑。
- 反对票也是一种复议手段。
...
- 另外维护已结算地址黑名单和已结算 `senderId` 黑名单，供 `isBanned(...)` 与前端批量读使用。
...
function isAddressBanned(
    uint256 groupId,
    address senderAddress
) external view returns (bool);

function isSenderIdBanned(
    uint256 groupId,
    uint256 senderId
) external view returns (bool);

function isAddressBannedBatch(
    uint256 groupId,
    address[] calldata senderAddresses
) external view returns (bool[] memory banned);

function isSenderIdBannedBatch(
    uint256 groupId,
    uint256[] calldata senderIds
) external view returns (bool[] memory banned);
...
- 票权源必须是合约，并实现 `IBanVoteWeightSource`；这是 Manager 与部署测试约束。
...
- `isBanned(...)`、`isAddressBannedBatch(...)`、`isSenderIdBannedBatch(...)` 仅读取已结算名单，不重新读取总票权。
- 通用批量黑名单读接口只返回发言 / 隐藏判断所需的布尔状态，不返回治理票数。
- 如果前端需要按指定地址或 `senderId` 解释治理黑名单原因，单个目标使用 `voteStatusBySenderAddress(...)` / `voteStatusBySenderId(...)`，批量目标使用 `voteStatusBySenderAddresses(...)` / `voteStatusBySenderIds(...)`，读取 `banned`、`supportWeight`、`opposeWeight`。
...
- `address[] addressBanList` 与对应 `indexPlusOne`
- `uint256[] senderIdBanList` 与对应 `indexPlusOne`
...
- 每个 voter 的 `supportBan`、`settledWeight`
...
- 不在 BanSource 内直接读取 LOVE20 token/action 状态。
- 对目标支持地址维度和 `senderId` 维度。
- 投票权重源必须由群类型 Manager 提供。
- 投票人必须是地址，不引入 `voterGroupId`。
- 不做全量重算接口，避免 `isBanned(...)` 或刷新写入依赖不可控循环。
