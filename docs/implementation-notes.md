...
`GROUP_CHAT_MAX_CONTENT_LENGTH` 缺省为 `4096`，写入 `GroupChat.MAX_CONTENT_LENGTH`。
`GROUP_CHAT_MAX_MENTIONED_SENDER_IDS` 缺省为 `32`，写入 `GroupChat.MAX_MENTIONED_SENDER_IDS`。
`GROUP_CHAT_MIN_SUPPORT_TO_OPPOSE_RATIO` 缺省为 `10`，表示支持禁言票必须超过反对禁言票的 10 倍。
